...
维护源码：`maintenance/wotr-2.7-offline`，GitHub 代码提交 `141b2ef65120dadd941a1128f42e2f37869eb56a`（与本地 `6dcc14c` 源码树相同；`73b78ab` 为文档整理，更晚修正见下文；映射见 `maintenance/github-import.json`）
...
- 先正常休息、记录实际资源数再通灵；资源充足时不应误加惩罚。
- 低资源边界由熟悉调试的维护者在独立测试档做：通灵前确认主灵体为空、资源确实在 2 附近，记录施放前后数值。不要假定“先降低资源再休息”还能保留低资源状态，也不要为此修改正式存档。
- 读档日志若出现 `Channel buff was not available during OnPostLoad`，把完整日志交给维护者，并记录灵体/能力是否正常；该提示本身不等于坏档。
